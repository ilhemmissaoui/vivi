...
import FinancementModal from "./FinanacementModal";
import {
  addEncaissementQuery,
  deleteEncaissement,
  getEcaissement,
...
const Encaissement = () => {
  ...
  const listEncaissements = async () => {
    try {
      const response = await getEcaissement(selectedProject, anneeId);
  ...
  const handleAddActivityClick = async () => {
    if (names) {
      ...
      const response = await addEncaissementQuery(
        selectedProject,
        anneeId,
        newEncaissement
      );
    ...
    <div>
      ...
      <div className="bp-container h-auto pb-5">
        ...
        <div className="flex items-start mt-3 ml-7">
          <button
            type="button"
            className="ml-6 bg-[#514495] hover:bg-[#5144959e] text-white font-bold py-1 px-4 rounded focus:outline-none"
