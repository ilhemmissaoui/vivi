...
class InvestissementRepository extends ServiceEntityRepository
{
    ...
    public function findInvestissementByAnneeInvestissementId($anneeInvestissementId): array
    {
        return $this->createQueryBuilder('i')
        ->andWhere('i.deleted = 0')
        ->leftJoin("i.investissementMontant","im")
        ->leftJoin("i.projetAnnees","ai")  
        ->andWhere('ai.id = :anneeInvestissementId')
        ->setParameter('anneeInvestissementId', $anneeInvestissementId)
        ->select('i.id, i.name,i.nature,i.duree,i.apportEnNature,i.name,im.montant')
        ->getQuery()
        ->getResult();
    }
